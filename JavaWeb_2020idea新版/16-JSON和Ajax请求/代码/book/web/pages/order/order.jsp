			...
			<c:forEach items="${requestScope.user_orders}" var="user_orders">
				<tr>
					...
					<td>
						...
						<c:if test="${user_orders.status eq 2}">已完成</c:if>
					</td>
					<td><a href="orderServlet?action=showOrderDetail&orderId=${user_orders.orderId}">查看详情</a></td>
					<td>
							<%--当订单状态为2时，才可以点击确认收货--%>
						<c:if test="${user_orders.status eq 1}">
							<a class="ReceivingGoods"
							   href="orderServlet?action=receiverOrder&orderId=${user_orders.orderId}">确认收货</a>
						</c:if>
							<%--否则无法点击，显示对应状态信息。--%>
